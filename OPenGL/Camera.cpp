...
void Camera::keyControl(bool* keys, GLfloat deltaTime)
{
    ...
    if (keys[GLFW_KEY_UP]) position += front * velocity;

    if (keys[GLFW_KEY_S]) position -= front * velocity;
    if (keys[GLFW_KEY_DOWN]) position -= front * velocity;

    if (keys[GLFW_KEY_A]) position -= right * velocity;
    if (keys[GLFW_KEY_LEFT]) position -= right * velocity;

    if (keys[GLFW_KEY_D]) position += right * velocity;
    if (keys[GLFW_KEY_RIGHT]) position += right * velocity;
